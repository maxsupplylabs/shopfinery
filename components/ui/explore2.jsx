...
useEffect(() => {
  ...
  const handleTabClick = (tabName) => {
    ...
    setCurrentPage(1); // Reset currentPage when switching tabs
          ...
          router.push(`current-batch-orders/?xp_tab=${tabName}`, { scroll: false });
  };
  const tabs = [
    {
      key: "recommendation",
  ...
  return (
    <div className="min-h-screen">
      <div className="flex justify-normal overflow-auto mb-4 px-2 sticky top-[6rem] md:top-[49px] z-50 bg-white py-1">
        {tabs.map((tab) => (
          <button
            key={tab.key}
